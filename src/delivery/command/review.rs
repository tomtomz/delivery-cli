...
pub fn run(review_opts: ReviewClapOptions) -> DeliveryResult<ExitCode> {
    let for_pipeline = review_opts.pipeline;
    let auto_bump = &review_opts.auto_bump;
    let no_open = &review_opts.no_open;
    let edit = &review_opts.edit;
    ...
    match try!(project::handle_review_result(&review, no_open)) {
