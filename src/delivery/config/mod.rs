...
impl Default for Config {
    fn default() -> Config {
        Config{
            ...
            saml: None,
            fips: None,
            fips_port: Some(String::from("32462")),
        }
    }
}
...
config_accessor_for!(fips_port, set_fips_port, "fips_port not set. set it in your cli.toml");
...
impl Config {
    ...
    pub fn delivery_git_ssh_url(&self) -> Result<String, DeliveryError> {
        match self.fips {
            ...
            None => self.delivery_git_ssh_standard_url()
        }
    }
    ...
    fn delivery_git_fips_enabled_url(&self) -> Result<String, DeliveryError> {
        let host_and_port = format!("{}:{}", "localhost", self.fips_port().unwrap());
        let u = try!(self.user());
        let e = try!(self.enterprise());
        let o = try!(self.organization());
        let p = try!(self.project());
    ...
    fn set_values_from_toml_table(table: toml::Table) -> Result<Config, DeliveryError> {
        ...
        config.fips_port = stringify_or("fips_port", &table, config.fips_port);
        return Ok(config);
    }
...
"#;
        ...
        match config_result {
            Ok(config) => {
                ...
                assert_eq!(Some(String::from("32462")), config.fips_port);
            },
            Err(e) => {
...
"#;
        ...
        match config_result {
            Ok(config) => {
                ...
                assert_eq!(Some("55555".to_string()), config.git_port);
            },
            Err(e) => {
                panic!("Failed to parse: {:?}", e.detail)
            }
